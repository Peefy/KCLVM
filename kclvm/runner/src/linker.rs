...
const KCL_RUNTIME_LIB_SHORT_NAME: &str = "kcl_runtime";
...
impl Command {
    ...
    pub(crate) fn unix_args(
        ...
        cmd.args(libs)
            ...
            .arg(&format!("-l{KCL_RUNTIME_LIB_SHORT_NAME}"));
        Ok(())
    }
    ...
    pub(crate) fn runtime_lib_path(&self, lib_path: &str) -> Result<String> {
        ...
        let lib_name = Self::get_runtime_lib_name();
    ...
    pub(crate) fn get_lib_link_path(&self) -> Result<String> {
        ...
        for folder in ["lib", "bin"] {
            let path = std::path::Path::new(&self.executable_root)
                .join(folder)
                .join(&Self::get_runtime_lib_name());
            if path.exists() {
                default_path = Some(
                    path.parent()
    ...
    /// Get KCLVM lib name
    pub(crate) fn get_runtime_lib_name() -> String {
        let suffix = Self::get_lib_suffix();
        if Self::is_windows() {
            format!("{KCL_RUNTIME_LIB_SHORT_NAME}{suffix}")
        } else {
            format!("lib{KCL_RUNTIME_LIB_SHORT_NAME}{suffix}")
        }
    }
